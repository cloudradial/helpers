...
All API calls use **Chrome JS `fetch()`** with the `x-functions-key` HTTP header for authentication. This is the only supported method — it works for all HTTP methods (GET, POST, PUT, PATCH, DELETE) with no URL length limits.

### Authentication

Credentials are loaded from `~/.cloudradial/config.json` (created by the setup wizard). The `x-functions-key` header is Azure Functions' standard authentication mechanism.
...
Where `base` and `key` come from the config file. Skills load these automatically.
...
```javascript
(async()=>{
  const r = await fetch(`${base}/update_resource`, {
    method: "POST",
    headers: {"x-functions-key": key, "Content-Type": "application/json"},
    body: JSON.stringify({
      resource_type: "article",
...
Cowork's bash/shell environment cannot reach external URLs. All API calls go through Chrome JS `fetch()` — never through bash scripts, curl, or Node.js in the shell.
