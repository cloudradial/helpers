...
function requireResource(args) {
    ...
    if (!config) {
        throw new Error(`Unknown resource_type: ${resourceType}. Valid: ${RESOURCE_TYPES.join(", ")}`);
    }
...
export const tools = [
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (resp.status === 401 || resp.status === 403) {
                throw new Error(`CloudRadial rejected those credentials (HTTP ${resp.status}). Double-check the public and private keys from your CloudRadial admin portal → Settings → API.`);
            }
            if (!resp.ok) {
    ...
    {
        name: "list_resources",
        description: "List any of 30 resource types with OData filtering, sorting, and pagination. Resource API caps each page at 200. Note: application_user has no OData listing and will error here — use get_resource instead.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const result = await callApi("GET", `/v2/odata/${config.odataPath}/$count`, filter ? { $filter: filter } : undefined);
    ...
    },
    {
        ...
        handler: async (args) => {
            const { resourceType, config } = requireResource(args);
            if (!config.itemPath)
                throw new Error(`create is not supported for ${resourceType}`);
    ...
    },
    {
        ...
        handler: async (args) => {
            const { resourceType, config } = requireResource(args);
            if (!config.itemPath)
                throw new Error(`update is not supported for ${resourceType}`);
    ...
    },
    {
        ...
        handler: async (args) => {
            const { resourceType, config } = requireResource(args);
            if (!config.itemPath)
                throw new Error(`delete is not supported for ${resourceType}`);
            let path;
            if (resourceType === "archive_item") {
                const archiveId = requireStr(args, "archive_id");
                const id = requireStr(args, "id");
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const filters = [];
            if (email)
                filters.push(`contains(tolower(email), '${escapeODataString(email)}')`);
            if (name) {
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const data = args.data || {};
            const result = await callApi("POST", `/v2/courseenrollment/${enrollmentId}/complete`, undefined, data);
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const queryObj = args.query || undefined;
            const query = queryObj
                ? Object.fromEntries(Object.entries(queryObj)
