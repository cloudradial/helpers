...
## Step 8: Configure the Plugin with Your Azure Function

The skill files contain placeholder values (`YOUR-FUNCTION-NAME` and `YOUR_FUNCTION_KEY`) that need to be replaced with your actual Azure Function URL and key from Steps 3 and 6. You have three options:

### Option A: Let Cowork Do It (Recommended)

Skip this step for now. After you install the plugin in Step 9, start a Cowork session and say **"Set up CloudRadial"**. The setup wizard will:

1. Detect the placeholders in your skill files automatically
2. Ask you for your Azure Function name and key right in chat
3. Update all three skill files for you
4. Test the connection
5. Confirm everything works

This is the easiest approach — no file editing required.

### Option B: Run the PowerShell Setup Script

If you'd rather configure before installing, run the setup script from the `cloudradial-ucp` folder:

```powershell
.\setup.ps1
```

The script prompts for your function name and key, then updates all three skill files automatically.

### Option C: Manual Find & Replace

Open these three files in a text editor:

1. `skills/setup/SKILL.md`
2. `skills/portal-lookup/SKILL.md`
3. `skills/content-management/SKILL.md`

Use **Find & Replace** (Ctrl+H) to make two replacements in each file:

| Find | Replace with |
|------|-------------|
| `YOUR-FUNCTION-NAME.azurewebsites.net` | Your function app name from Step 3 (e.g., `my-cloudradial-mcp.azurewebsites.net`) |
| `YOUR_FUNCTION_KEY` | Your function key from Step 6 |

**Use "Replace All"** — these placeholders appear many times in each file.

**Verify:** Search for `YOUR_FUNCTION_KEY` and `YOUR-FUNCTION-NAME` across all three files. If any remain, you missed one.

---

## Step 9: Package and Install the Plugin in Cowork

Now you need to turn the plugin folder into a `.plugin` file and install it in Claude Desktop.

### Package the plugin

1. Open File Explorer and navigate to the `cloudradial-ucp` folder
2. Select **everything inside the folder** (not the folder itself) — you should see `.claude-plugin/`, `skills/`, `references/`, `README.md`, etc.
3. Right-click → **Compress to ZIP file** (or "Send to → Compressed folder")
4. Rename the resulting `.zip` file to `cloudradial-ucp.plugin` (change the extension from `.zip` to `.plugin`)

**What to include:** `.claude-plugin/`, `skills/`, `references/`, `README.md`, `CAPABILITIES.md`, `DEPLOYMENT.md`

**What NOT to include:** `azure-mcp-server/` (this is the server code, not needed in the plugin), `node_modules/`, `.git/`

### Install in Cowork

1. Open **Claude Desktop**
2. Start a new conversation in **Cowork mode**
3. Drag the `cloudradial-ucp.plugin` file into the chat window
4. Claude will install the plugin and confirm

---

## Step 10: First Use — Verify Everything Works

**Important first step:** Cowork's `web_fetch` tool has a "provenance" requirement — it can only call URLs that you've pasted into chat at least once. In your first session, paste this URL directly into the chat (substituting your values):

```
https://YOUR-FUNCTION-NAME.azurewebsites.net/api/cloudradial/search_companies?code=YOUR_FUNCTION_KEY&name=test
```

Claude will call the URL and return company data from your CloudRadial portal. If this works, you're fully connected.

Now try natural language:
...
You only need to paste the URL once per Cowork session to seed provenance for `web_fetch`. Write operations use Chrome JS `fetch()` which has no provenance restriction — they just work.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `web_fetch` refuses to fetch URL | URL provenance not seeded | Paste the full URL into chat once (Step 10) |
| `web_fetch` returns 403 on complex queries | URL too long or spaces in OData filter | Claude will automatically switch to Chrome JS `fetch()` |
| Write operations fail ("Chrome not connected") | Claude in Chrome not installed/connected | Install the extension (Step 0h) and connect it |
...
| Bash/shell can't reach the API | Cowork sandbox is network-isolated | Expected — all calls go through `web_fetch` or Chrome JS, never bash |
